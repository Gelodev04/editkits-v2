...
export type MultipleUploadModalProps = {
  uploadModal: boolean;
  setUploadModal: (e: React.SetStateAction<boolean>) => void;
  videoRef?: React.Ref<HTMLVideoElement>;
  fileIndex: number;
  setFile?: (file: any, index: number) => void;
  setFileId?: (fileId: string, index: number) => void;
  upload?: any;
  isUploading?: boolean;
  setIsUploading?: any;
...
export default function UploadMultipleFileModal(props: MultipleUploadModalProps) {
  ...
  React.useEffect(() => {
    ...
    try {
      ...
      if (fileStatusData.status === 'EXPIRED') {
        ...
        if (props.setFile) {
          // Create and set the new mock file
          ...
          props.setFile(mockFile, props.fileIndex);

          // Now set the fileId
          if (props.setFileId) {
            props.setFileId(fileIdToCheck, props.fileIndex);
          }
        ...
        >
          ...
          <div className="px-8 py-6 relative border-b border-gray-100 dark:border-gray-800">
            ...
            <p className="mt-1 text-gray-700 dark:text-gray-400 text-sm">
              Select a video or image file to upload and join
            </p>
          </div>
                ...
                <>
                  ...
                  <div className="mt-2 mb-2 text-center">
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                      Supported formats: MP4, MOV, AVI, WEBM, JPG, PNG, GIF and many more.
                    </p>
                  </div>
                </>
              </div>
