...
function ButtonStyle(variant: string, filled?: boolean, disabled?: boolean, width?: number, border?: boolean, bold?: boolean, className?: string, font?: string, fontWeight?: string) {
  if (variant === "primary") {
    return ` ${className} ${bold ? "font-semibold text-[#2c2c2c]": `${fontWeight ?? "font-bold"} text-[#000]`} hover:scale-105 transition-transform duration-300 hover:text-[#273266] text-sm flex justify-center`
  } else if (variant === "secondary") {
    return `${className} ${!className?.includes('py') ? "py-4" : className} font-bold font-lato text-lg w-full ${filled && !disabled ? "bg-[#273266] text-white hover:bg-[#273266]" : filled && disabled ? "bg-neutral-50 text-[#4f4f4f]" : "hover:bg-sky-100 text-[#4f4f4f]"} ${!disabled && "hover:scale-105 transition duration-300 cursor-pointer"} border-2 border-zinc-100 flex justify-center items-center gap-2 rounded-full`;
  } else if (variant === "outlined") {
    return ` ${className} font-bold py-4 font-lato text-lg w-full ${filled && !disabled ? "bg-[#148CFC] text-white" : "bg-[#fafafa] text-[#4f4f4f]" } ${!disabled && "hover:scale-105 transition duration-300 cursor-pointer hover:bg-[#273266]"} border-2 border-zinc-100 flex justify-center items-center gap-4 rounded-full`;
