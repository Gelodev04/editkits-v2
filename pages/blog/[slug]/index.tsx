  ...
  function boldText(text) {
    ...
    return parts.map((part, index) =>
      part.startsWith('**') && part.endsWith('**') ? (
        <span
          className="font-lato font-bold text-base leading-[24px] text-[#4f4f4f] dark:text-[#b0b0b0] py-[12px] "
          key={index}
        >
          {part.slice(2, -2)}
        </span>
      ...
      {article?.content.map(content => {
        if (content.type === 'heading') {
          if (content.style === 'large') {
            return (
              <h1 className="font-montserrat font-bold text-[40px] sm:text-[48px] leading-[64px] text-[#2c2c2c] dark:text-[#d5d7da] py-[12px]">
                {boldText(content.value)}
              </h1>
            );
          }
        ...
        if (content.type === 'text') {
          return (
            <p className="font-lato font-normal text-base  leading-[24px] text-[#4f4f4f] dark:text-[#d5d7da] py-[12px]">
              {boldText(content.value)}
            </p>
          );
        }
        ...
        if (content.type === 'divider') {
          return (
            <div className="py-[12px]">
              <hr className="border-b border-gray-200 dark:border-gray-800 border-[2px]" />
            </div>
          );
        }

        if (content.type === 'space') {
          return <div style={{ paddingTop: content.px }} />;
        }
